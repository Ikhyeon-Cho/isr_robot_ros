...
namespace isr_robot
{
bool M4::ConnectRobot(const std::string& port, const int baudrate)
{
  try
  {
    serial_.open(port);
  ...
  {
    std::cout << "\033[1;31mBaudrate Error: " << e.what() << "\033[0m" << std::endl;
    return false;
  }
...
void M4::DisconnectRobot(void)
{
  serial_.close();
}

bool M4::Initialize(void)
{
...
bool M4::SetVelocity(double leftWheelVel_MPS, double rightWheelVel_MPS)  // m/s, m/s
{
  // �¿� ���� ������ ����� ������ ȸ���ӵ��� RPM ������ ����Ѵ�.
...
bool M4::SetVelocityVW(double linearVel_MPS, double angularVel_RPS)  // m/s, rad/s
{
...
bool M4::ReadVelocity_ADCApprox(void)
{
...
bool M4::ReadRobotStatus(uint8_t& motorEnableStatus, uint8_t& motorStopStatus, uint8_t& emergencyButtonPressed)
{
...
void M4::DeadReckoning(long dl_pulse, long dr_pulse)
{
...
bool M4::SendData(uint8_t command, uint8_t numparam, uint8_t* params)
{
...
std::vector<uint8_t> M4::ReceiveData(uint8_t& command)
{
...
std::vector<uint8_t> M4::Word2Bytes(int16_t dat)
{
...
std::vector<uint8_t> M4::Long2Bytes(int32_t dat)
{
...
int16_t M4::Bytes2Word(uint8_t* data)
{
  int dat_1 = data[0] << 8;
  int dat_2 = data[1];
  int dat = dat_1 | dat_2;
...
int32_t M4::Bytes2Long(uint8_t* data)
{
  int dat_1 = data[0] << 24;
  int dat_2 = data[1] << 16;
  int dat_3 = data[2] << 8;
